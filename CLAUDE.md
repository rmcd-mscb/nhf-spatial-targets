...
version/hash-stamped projection of (`config.yml` × `defaults.py` × `fabric.json`),
regenerated by `validate`.

### Determinism rules (the rebuild path is a pure function of disk)

- **No `datetime.now()` anywhere reachable from `rebuild_manifest`.** Every
  timestamp comes from file `mtime` via `lineage.iso_from_mtime`. `validate`
  and live capture *may* call `now()` — they are not the rebuild path. A test
  guards the rebuild module against a `datetime.now` reference.
- **Deterministic JSON ordering.** Sources are emitted with sorted keys; steps
  are sorted by `lineage.step_sort_key` (`(kind-rank, source_key,
  smallest-output-path)` — the tiebreaker is the lexicographically smallest
  output path via `min()`, not positional list order). Same disk + catalog +
  code → **byte-identical** manifest (modulo the opt-in `--compute-sha256`).
- **Honesty tags.** Every regenerated record carries
  `provenance: "reconstructed"`; the manifest carries
  `manifest_schema_version` and the effective config
  `effective_config_schema_version`.
- **Non-clobbering of identity fields.** Regenerate rewrites only the derived
  catalog (`sources`, `steps`); `created_utc`, the `fabric` authorship block,
  and any `release` config are read-merged from the existing manifest, never
  re-minted.

### When you add a source / variable / target / stage

Modeled on the three-point "Config schema additions" checklist above:

1. **Extend the `rebuild_manifest` projection and its test in the same PR.**
   The projection is generic and catalog-keyed (it walks `<datastore>/<key>/`,
   `data/aggregated/<key>/`, and `targets/*.nc` with no per-source hook), so a
   new catalog source usually needs no projection change — but a new *stage* or
   a new on-disk *layout* does. Add the matching case to
   `tests/test_rebuild_manifest.py`.
2. **Bump `CURRENT_MANIFEST_SCHEMA_VERSION` (lineage.py) only when the manifest
   top-level *shape* changes** — not when sources/steps *content* changes.
   `read_manifest` defaults a version-less manifest to `0` so pre-version
   manifests stay detectable. Bump `EFFECTIVE_CONFIG_SCHEMA_VERSION`
   (validate.py) under the same rule for the effective config.
3. **Never write a manifest or effective config that bypasses the shared
   skeleton / projection.** `manifest.json` is written only by
   `lineage._new_manifest_skeleton` + `atomic_write_manifest` (used by both
   `validate._write_manifest` and `rebuild_manifest`); `config.effective.yml`
   only by `validate._write_effective_config`. A second inline writer recreates
   the two-skeleton drift #279 removed.

### Per-fabric / per-release guarantee

- **Every fabric gets a manifest:** `validate` mints one per project (each
  project is one fabric); no fabric registry exists (discoverability is derived
  on demand by scanning for `fabric.json` / `manifest.json`).
- **Every release child carries a complete one:** `release.payload.stage_fabric_child`
  hard-errors if the manifest is absent (never a silent skip).
- **Publish is gated on completeness:** `release.publish._preflight_provenance_complete`
  runs `rebuild_manifest(..., dry_run=True)` and refuses to publish if the
  on-disk manifest differs from the projection or is incomplete (the #277 ∪
  #278 source union, a published target NC with no `target` step, drift).
  `--allow-incomplete-sources` is the deliberate, logged override; it does
  **not** bypass the always-fatal structural checks (schema version, missing
  `fabric` block, empty `steps[]`, an unreadable/corrupt published target NC).
- **`config.effective.yml` is staleness-gated:** `_preflight_effective_config_current`
  refuses to publish when the stamped source-config hash no longer matches
  `config.yml` (the operator edited config without re-running `validate`).
