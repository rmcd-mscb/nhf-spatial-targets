...
- SCA CI-bounds formula — `https://github.com/rmcd-mscb/nhf-spatial-targets/blob/main/docs/references/PRMSobjfun.f90` (the calibration-side objective function) landed in this repo at [`docs/https://github.com/rmcd-mscb/nhf-spatial-targets/blob/main/docs/references/PRMSobjfun.f90`](https://github.com/rmcd-mscb/nhf-spatial-targets/blob/main/docs/references/PRMSobjfun.f90) on 2026-05-24 via a collaborator. The SCA bound is built inside `calcSCA` (lines 1052-1061) from the MOD10C1 `(SCA_obs, CI)` pair when `CI > 70`: `lower = (CI/100) * SCA_obs`, `upper = lower + (1 - CI/100)`. July/August forced to `(0, 0)`. The Fortran gate is `CI >= 70` (`PRMSobjfun.f90:1052`); `targets/sca.py` implements this verbatim as `ci >= ci_threshold`. Full crib sheet keyed to this repo: [`prmsobjfun-summary.md`](prmsobjfun-summary.md).
- SCA single-source lock — `targets/sca.py` originally hardcoded `sources == [mod10c1_v061]`. Closed by #237 (PR-D): SCA is now a per-source interval registry combined NaN-aware (`lower = nanmin`, `upper = nanmax`), with `ua_swe`'s depth-derived `snow_covered_fraction` as a second source (a degenerate `[v, v]` interval reaching back to WY 1982, widening the pre-2000 bound where MOD10C1 (2000+) is the other source). Two config knobs gate the combine: `snow_covered_area.forced_zero_combined` (default `true` — zero the combined bound in Jul/Aug; `false` zeros only the mod10c1 contribution so ua_swe's summer fraction survives) and `min_sources_for_bound` (default `1`; `2` NaNs the bound where fewer than two sources are finite). An explicit mod10c1-only config still builds the single-source bound byte-for-byte (the default `sources` is now two-source).
