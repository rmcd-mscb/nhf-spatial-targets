...
- SCA CI-bounds formula — `PRMSobjfun.f90` (the calibration-side objective function) landed in this repo at [`docs/references/PRMSobjfun.f90`](PRMSobjfun.f90) on 2026-05-24 via a collaborator. The SCA bound is built inside `calcSCA` (lines 1043-1050) from the MOD10C1 `(SCA_obs, CI)` pair when `CI > 70`: `lower = (CI/100) * SCA_obs`, `upper = lower + (1 - CI/100)`. July/August forced to `(0, 0)`. `targets/sca.py` can be promoted from stub to implementation. Full crib sheet keyed to this repo: [`prmsobjfun-summary.md`](prmsobjfun-summary.md).
