...
4. **No fetch-side re-projection.** Re-projecting the Reitz consolidated NC to EPSG:4326 at fetch time was considered as an alternative to declaring `source_crs="EPSG:4269"` in the adapter. Rejected for four reasons:

   - **The CRS difference is negligible in CONUS.** NAD83 (EPSG:4269) and WGS84 (EPSG:4326) are both geographic coordinate systems on near-identical reference ellipsoids (GRS80 vs WGS84 ellipsoid; semi-major axis differs by ~0.1 mm). For points in CONUS the coordinate values for the same physical location differ by < 2 m horizontally — well below Reitz's 800 m grid resolution and far below the HRU-polygon scale, so the area-weighted aggregation is unaffected to several decimal places.

   - **Re-projection is not free.** It's a resampling operation on a continuous raster, which forces a methodological choice (nearest / bilinear / cubic) and introduces interpolation artifacts at every cell. Picking a method becomes another decision the project has to justify in the recipes doc, when the underlying need ("make gdptools happy") doesn't require it.

   - **gdptools handles datum mismatch correctly.** `WeightGen`/`AggGen` accept `source_crs` and `target_crs` independently and reconcile them via pyproj. Declaring `source_crs="EPSG:4269"` in the `SourceAdapter` is a one-line change with the same numerical effect as re-projecting first, but without touching the data on disk.

   - **Re-projection invalidates every populated datastore.** The user's HPC datastore already has `reitz2017_consolidated.nc` fetched. A re-projection would change the on-disk values (even if only by ~1 m in coordinate space) and force everyone to re-fetch — wasted bandwidth, wasted time, and a new opportunity for divergence between developers' local fetches.

   The precedent in this repo is consistent: when a source's on-disk CRS or coordinate convention differs from the EPSG:4326 default, the fix lands either in CF-metadata application at consolidation time (e.g. NCEP/NCAR's 0–360 longitude wrap, PR #57) or in the adapter declaration (`x_coord`, `y_coord`, `source_crs` overrides). The data on disk is not re-projected. Reitz follows the second pattern.
