...
def check_manifest_schema(project_dir: Path) -> int | None:
    ...
    # Validate shape, not just parseability: a valid-JSON-but-non-object
    # payload (``[]``, ``null``, ``42``) would otherwise raise AttributeError
    # on ``.get`` and escape the CLI's (FileNotFoundError, ValueError) handler.
    if not isinstance(manifest, dict):
        raise ValueError(
            f"manifest.json at {manifest_path} is not a JSON object "
            f"(got {type(manifest).__name__})."
        )
    version = manifest.get("manifest_schema_version", 0)
    # ``bool`` is an ``int`` subclass, so exclude it explicitly. A non-integer
    # version must fail loud, not compare silently (a float like 2.5 would
    # otherwise masquerade as "in sync") nor escape as an uncaught TypeError.
    if not isinstance(version, int) or isinstance(version, bool):
        raise ValueError(
            f"manifest_schema_version in {manifest_path} is {version!r} "
            f"({type(version).__name__}); expected an integer."
        )
