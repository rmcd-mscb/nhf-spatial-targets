...
# Bumped ONLY when the config.effective.yml top-level *shape* changes. A NEW
...
def _write_effective_config(workdir: Path, merged: dict) -> Path:
    ...
    The file is stamped with a dedicated ``_effective_config_meta`` block
    carrying :data:`EFFECTIVE_CONFIG_SCHEMA_VERSION` and the sha256 of the
    *verbatim* ``config.yml`` bytes, so the publish gate can detect a stale
    effective config (``config.yml`` changed since this was generated) as an
    explicit inconsistency rather than silent drift. The hash is over the raw
    on-disk ``config.yml`` bytes, not the merged dict, so "stale" means exactly
    "the operator edited ``config.yml`` without re-running validate". No
    wall-clock timestamp is stamped, so the artifact stays reproducible.
    ...
    source_config_sha256 = hashlib.sha256(
        (workdir / "config.yml").read_bytes()
    ).hexdigest()
    # Inject the version/hash stamp into a COPY: the live ``merged`` dict is
    # reused downstream (manifest step params, ``_write_manifest``) and must not
    # gain the meta block. ``sort_keys=False`` preserves insertion order, so the
    # meta block dumps first, directly under the AUTOGENERATED header.
    stamped = {
