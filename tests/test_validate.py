...
# ---------------------------------------------------------------------------
# Output files: config.effective.yml
# ---------------------------------------------------------------------------


def test_validate_writes_effective_config(
    tmp_path, minimal_fabric, no_system_cred_checks
):
    """validate_workspace must produce config.effective.yml as a side-effect."""
    _full_setup(tmp_path, minimal_fabric)
    validate_workspace(tmp_path)

    out = tmp_path / "config.effective.yml"
    assert out.exists()
    text = out.read_text()
